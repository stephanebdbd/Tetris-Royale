...
Comme représenté par la figure 2, une fois que l'utilisateur s'est connecté ou a créé un compte, il accède à la page du menu principal. Cette page constitue le point central de l'application. En effet, l'utilisateur pourra jouer (soit créer ou rejoindre une partie). Il pourra également aller dans le menu d'amis, visualiser le classement général des meilleurs joueurs, avoir accès au chat avec ses différents amis, et, enfin, se déconnecter de son compte actif.
...
Dans le menu des amis, l'utilisateur aura la possibilité de gérer sa liste d'amis. Il pourra ajouter ou supprimer un ami et bien entendu, retourner au menu principal.
...
Les comptes créés seront enregistrés dans la base de données avec leurs informations correspondantes par le système. Chaque utilisateur se trouvera dans la base de données SQL possédant: un ID, un pseudonyme, un mot de passe, un meilleur score et un ID d'avatar.
...
Chaque amis d'un utilisateur se trouvent dans la base de données.
...
Le programme fait en sorte qu'un utilisateur ne voit que son plan de jeu, son score, son prochain tetramino et la grille des autres joueurs.

\subsubsection{Apparition des tétraminos}
Le système fera apparaître des tétraminos de manière aléatoire pour l'utilisateur. Le tétramino actuel se trouvera dans la grille, tandis que le prochain tétramino sera affiché à côté de l'espace de jeu. Ils apparaîtront en haut de la grille et tomberont progressivement un à un.

\subsubsection{Fin de la partie}
Lorsque le programme détecte une fin de partie, tous les joueurs recevront un message indiquant s'ils ont gangé ou perdu ainsi que leurs scores respectifs.
...
\subsection{Diagrammes de Classe du serveur}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{serveur1.png}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{serveur2.png}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{serveur3.png}
\end{figure}

\subsection{Diagrammes de Classe du serveur/database}
...
\subsection{Diagrammes de Classe du client}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{client1.png}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{client3.png}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{client4.png}
\end{figure}
...
Si le joueur décide de créer une partie et il choisit le mode Endless, et puis la fenêtre affiche la grille du jeu avec un score à zéro et avec un tetramino qui apparaît en haut de la grille et qui tombe progressivement. Avant que le tetramino arrive à la base de la grille, le joueur peut le déplacer et le tourner. Après chaque placement du tetramino le GameServer vérifie chaque fois s'il y a une ou plusieurs lignes complètes, si c'est le cas, le score va être augmenter et également la vitesse des tetraminos qui vont arriver après. le même scénario se répète jusqu'à un tetramino touche le haut de la grille et le jeu se termine par l'affichage de le score obtenu dans ce partie et s'il est supérieur au meilleur score, le GameServer l'enregistre dans la base de donnée comme le nouveau meilleur score.
\begin{figure}[H]
    \centering
...
Ce diagramme est presque le même que du mode classic et duel, La seul différence c'est le moyen d'obtenir des malus. Chaque fois le joueur complète des lignes le GameServer met-à-jour le score et l'énergie de joueur. S'il a recueille assez d'énergie, la fenêtre demande à lui de choisir entre malus ou bonus, si le joueur choisit de béneficier d'un bonus le GameServer va l'appliquer, au cas contraire le GameServer va envoyer les malus à l'adversaire choisi par le joueur. 
\begin{figure}[H]
    \centering
...
\begin{figure}[H]
    ...
    \includegraphics[width=0.7\textwidth]{ActivityDiagram/Creation_d_une_partie.pdf}
    \label{Figure 15}
\end{figure}
\newpage
\subsubsection{L'envoi des malus - mode Classic et Duel}
Ce diagramme montre le mécanisme de malus dans le mode Classique et le mode Duel du Tetris Royale : lorsque le joueur complète des lignes, le système vérifie leur nombre. Si au moins 2 lignes sont complétées, des malus sont envoyés aux joueurs choisis par le système (1 ligne de malus pour 2 lignes complétées, 2 pour 3, et 4 pour 4).Le joueur valide le choix de joueur cible.Si le choix n’est pas validé, le joueur peut sélectionner une autre cible avant l’envoi du malus par le système. 
\begin{figure}[H]
    \centering
